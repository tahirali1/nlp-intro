import unittest
from nlp.tf_idf_model import TfidfFModel


class TestTfdfModel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model = TfidfFModel(long_str)

    def test_remove_stop_words(self):
        dataset = self.model.prepare_data()

        word_count = self.model.create_histogram(dataset)

        freq_words = self.model.frequent_words(word_count)

        idf_matrix = self.model.prepare_idf_matrix(dataset, freq_words)

        tf_matrix = self.model.prepare_tf_matrix(dataset, freq_words)

        score_matrix = self.model.td_idf_calculation(idf_matrix, tf_matrix)

        tfidf_model = self.model.build_tdidf_model(score_matrix)

        self.assertEqual(21, len(tfidf_model))
        self.assertEqual(100, len(tfidf_model[0]))
